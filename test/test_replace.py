...
def test_replacing_one():
    ...
    engine.push(TableTest)

    obj_a = TableTest(column_one="Hello World!", column_two=42)
    obj_b = TableTest(column_one="Goodbye World!", column_two=-123)
    ...
    in_db_obj_a = engine.query(TableTest).where(TableTest.c.id == obj_a_inital_id).all()
...
def test_replacing_many():
    ...
    in_db_obj_a = engine.query(TableTest).where(TableTest.c.id == obj_a_inital_id).all()
    in_db_obj_b = engine.query(TableTest).where(TableTest.c.id == obj_b_inital_id).all()
    assert in_db_obj_a[0].column_one == "Once upon a time"
    assert in_db_obj_a[0].column_two == 42
    assert in_db_obj_a[0].id == obj_a_inital_id
